...
namespace Ecommerce.Application.Tests
{
    ...
    public class AdminTaxCommandHandlerTests
    {
        private ApplicationDbContext CreateInMemoryContext()
        {
            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
        ...
        [Fact]
        public async Task CreateTaxCategory_CreatesCategoryWithRates()
        {
            using var ctx = CreateInMemoryContext();
            var handler = new CreateTaxCategoryCommandHandler(ctx, CreateMapper());
            ...
            var command = new CreateTaxCategoryCommand
            {
                ...
                IsActive = true,
                Rates = new List<CreateTaxRateCommand>
                {
                    new CreateTaxRateCommand { CountryCode = "US", RegionCode = "CA", Rate = 0.0825m, IsActive = true }
                }
            };
            ...
            Assert.NotNull(result);
            Assert.Equal("Standard Goods", result.Name);
            Assert.True(result.IsActive);
            Assert.Single(result.Rates);
            Assert.Equal(0.0825m, result.Rates[0].Rate);
        }

        [Fact]
        public async Task CreateTaxCategory_DuplicateName_Throws()
        {
            using var ctx = CreateInMemoryContext();

            await ctx.TaxCategories.AddAsync(new TaxCategory
            {
                Id = Guid.NewGuid(),
                Name = "Dup",
                IsActive = true,
            ...
            var handler = new CreateTaxCategoryCommandHandler(ctx, CreateMapper());
            var command = new CreateTaxCategoryCommand { Name = "Dup", IsActive = true };
            await Assert.ThrowsAsync<DomainException>(() => handler.Handle(command));
        }

        [Fact]
        public async Task UpdateTaxCategory_UpdatesFieldsAndRates()
        {
            using var ctx = CreateInMemoryContext();
            ...
            var handler = new UpdateTaxCategoryCommandHandler(ctx, CreateMapper());
            var command = new UpdateTaxCategoryCommand
            {
                Id = category.Id,
                Name = "Updated",
                IsActive = false,
                RowVersion = category.RowVersion,
                Rates = new List<UpdateTaxRateCommand>
                {
                    new UpdateTaxRateCommand { Id = rate.Id, CountryCode = "US", RegionCode = "NY", Rate = 0.07m, IsActive = true }
                }
            };
            ...
            Assert.False(result.IsActive);
            Assert.Single(result.Rates);
            Assert.Equal(0.07m, result.Rates[0].Rate);
        }

        [Fact]
        public async Task CreateTaxRateOnly_DuplicateRegion_Throws()
        {
            using var ctx = CreateInMemoryContext();
            ...
            var handler = new CreateTaxRateOnlyCommandHandler(ctx, CreateMapper());
            var command = new CreateTaxRateOnlyCommand
            {
                TaxCategoryId = category.Id,
                CountryCode = "US",
                RegionCode = "CA",
        ...
        [Fact]
        public async Task GetTaxCategories_ReturnsPagedResults()
        {
            using var ctx = CreateInMemoryContext();
            ...
            var handler = new GetAdminTaxCategoriesQueryHandler(ctx, CreateMapper());
            var query = new GetAdminTaxCategoriesQuery { Page = 1, PageSize = 10 };
        ...
        [Fact]
        public async Task GetTaxRateById_NotFound_Throws()
        {
            using var ctx = CreateInMemoryContext();
            var handler = new GetAdminTaxRateByIdQueryHandler(ctx, CreateMapper());
            await Assert.ThrowsAsync<NotFoundException>(() => handler.Handle(new GetAdminTaxRateByIdQuery { Id = Guid.NewGuid() }));
        }
    }
}